...
async def _register_route(effect: RegisterHttpRoute) -> None:
    app = _require_fastapi(effect.app)
    if not callable(effect.endpoint):
        raise TypeError(f"Endpoint must be callable, got {type(effect.endpoint)}")
    # Register route using FastAPI's public API.
    # Note: effect.endpoint is typed as `object` in the pure effect DSL to allow
    # flexibility in endpoint signatures. The callable() check above validates
    # it at runtime. MyPy override in pyproject.toml permits the type mismatch
    # at this interpreter boundary.
    app.add_api_route(
        path=effect.path,
