...
def main() -> int:
    steps: list[tuple[str, list[str] | None]] = [
        (
            "STEP 1/5: Black",
            ["black", "src/python/effectful", "tests", "src/python/effectful_tools"],
        ),
        (
            "STEP 2/5: Ruff",
            ["ruff", "check", "src/python/effectful", "tests", "src/python/effectful_tools"],
        ),
        ("STEP 3/5: MyPy", ["mypy"]),  # Uses paths from pyproject.toml [tool.mypy] files config
