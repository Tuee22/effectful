# Formal methods for distributed consensus & blockchains (with mixed trust)
...
2. [System model: what must be specified](#system-model-what-must-be-specified)
3. [Fault models & “trust levels” across nodes](#fault-models--trust-levels-across-nodes)
4. [Consensus problem taxonomy (what you prove)](#consensus-problem-taxonomy-what-you-prove)
5. [Consensus families: Paxos, PBFT, Raft/Zab, modern BFT](#consensus-families-paxos-pbft-raftzab-modern-bft)
6. [Hybridizing trust: “Paxos ↔ PBFT” and beyond](#hybridizing-trust-paxos--pbft-and-beyond)
7. [Blockchains vs classical consensus (formal-system vs classical)](#blockchains-vs-classical-consensus-formal-system-vs-classical)
8. [DAG systems + blockchains: composition & proof boundaries](#dag-systems--blockchains-composition--proof-boundaries)
9. [Where the proof boundary falls (TCB & assumptions)](#where-the-proof-boundary-falls-tcb--assumptions)
10. [A practical TLA+ workflow for consensus systems](#a-practical-tla-workflow-for-consensus-systems)
11. [Appendices: checklists & interface contracts](#appendices-checklists--interface-contracts)
...
2) **Explicit liveness assumptions** (what you *must* assume about timing/fairness).  
3) **Small, auditable trust base** (crypto/hardware/network assumptions clearly isolated).  
4) **Compositionality** (prove layers separately, then prove the interface contract).
...
2) **DAG commitments + chain anchoring**
...
4) **Hardware / trusted modules (if any)**
   - TEEs, monotonic counters, HSMs; these become critical assumptions
5) **Implementation & compilation chain**
...
2) Paxos-like core
3) multi-shot log replication
4) leader optimization
5) reconfiguration
6) persistence / crash-recovery
7) performance features (batching, pipelining)
...
Formal methods (TLA+)
- Lamport, *Specifying Systems* (TLA+) and related material.  
- Cousineau et al., *TLA+ Proofs* (TLAPS).

Bridging Paxos and PBFT formally
- Lamport, *Byzantizing Paxos by Refinement* (and linked machine-checked specs/proofs).

Blockchain-era BFT and DAG layers
- Yin et al., *HotStuff* (bridge between classical BFT and blockchains).  
- Danezis et al., *Narwhal and Tusk* (DAG mempool + BFT ordering).  
- Keidar et al., *DAG-Rider* (“All You Need is DAG”) (DAG-based atomic broadcast).

---

## Source URLs (for convenience)

- Byzantine Generals PDF: https://lamport.azurewebsites.net/pubs/byz.pdf  
- Paxos / Part-Time Parliament PDF: https://lamport.azurewebsites.net/pubs/lamport-paxos.pdf  
- PBFT PDF: https://css.csail.mit.edu/6.824/2014/papers/castro-practicalbft.pdf  
...
- Byzantizing Paxos by Refinement (links to specs & proofs): https://lamport.azurewebsites.net/tla/byzpaxos.html  
- HotStuff (arXiv): https://arxiv.org/abs/1803.05069  
- Narwhal & Tusk (arXiv): https://arxiv.org/abs/2105.11827  
- All You Need is DAG / DAG-Rider (arXiv): https://arxiv.org/abs/2102.08325  
