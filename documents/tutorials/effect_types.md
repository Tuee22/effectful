...
- Docker workflow running per [Docker Workflow](../engineering/docker_workflow.md); commands run via `docker compose ... exec effectful`.
- Completed [Tutorial 01: Quickstart](quickstart.md) to run and test generator-based programs.
- Familiarity with core architecture from [architecture.md](../engineering/architecture.md) and purity/type rules in [code_quality.md](../engineering/code_quality.md).

## Learning Objectives

- Understand the six effect families and when to use each.
- Author generator-based programs that yield different effect types with explicit return values.
- Compose effects safely with exhaustive pattern matching and type-safe error handling.

## Step 1: Review effect categories
...
See [Tutorial 08: Messaging Effects](messaging_effects.md) for comprehensive coverage including `AcknowledgeMessage` and `NegativeAcknowledge`.

## Step 6: Storage effects
...
See [Tutorial 09: Storage Effects](storage_effects.md) for comprehensive coverage including `DeleteObject` and `ListObjects`.
...
See [Tutorial 10: Auth Effects](auth_effects.md) for comprehensive coverage including `RefreshToken`, `RevokeToken`, and `GetUserByEmail`.

## Step 8: Compose effects
...
- Reviewed the six effect families and their canonical use cases.
- Practiced generator patterns for yielding each effect type with explicit return values.
- Applied composition, error handling, and exhaustive pattern matching to keep programs type-safe.

## Next Steps

- [Tutorial 03: ADTs and Result Types](adts_and_results.md) - Deep dive into type safety
