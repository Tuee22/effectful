...
class TestMessagingWorkflowIntegration:
    ...
    async def test_publish_timeout_workflow(self, mocker: MockerFixture) -> None:
        """Workflow handles publish timeout failure gracefully via ADT matching."""
        # Create mocks
        mock_producer = mocker.AsyncMock(spec=MessageProducer)
        mock_consumer = mocker.AsyncMock(spec=MessageConsumer)
        ...
        # Define workflow that handles PublishFailure ADT
        def publish_timeout_program() -> Generator[AllEffects, EffectResult, str]:
            """Try to publish, handle timeout via pattern matching."""
            publish_result = yield PublishMessage(topic="test-topic", payload=b"data")

            match publish_result:
                case str():
                    return f"published: {publish_result}"
                case PublishFailure(reason=reason):
        ...
        match result:
            ...
            case Err(error):
                pytest.fail(f"Expected Ok('failed: timeout'), got Err({error})")

    @pytest.mark.asyncio()
    async def test_publish_quota_exceeded_workflow(self, mocker: MockerFixture) -> None:
        """Workflow handles quota exceeded failure via ADT matching."""
        # Create mocks
        mock_producer = mocker.AsyncMock(spec=MessageProducer)
        mock_consumer = mocker.AsyncMock(spec=MessageConsumer)
        ...
        # Define workflow that handles PublishFailure ADT
        def publish_quota_program() -> Generator[AllEffects, EffectResult, str]:
            """Try to publish, handle quota exceeded via pattern matching."""
            publish_result = yield PublishMessage(topic="test-topic", payload=b"data")
        ...
        match result:
            case Ok(outcome):
                assert outcome == "failed: quota_exceeded"
            case Err(error):
                pytest.fail(f"Expected Ok('failed: quota_exceeded'), got Err({error})")

    @pytest.mark.asyncio()
    async def test_publish_topic_not_found_workflow(self, mocker: MockerFixture) -> None:
        """Workflow handles topic not found failure via ADT matching."""
        # Create mocks
        mock_producer = mocker.AsyncMock(spec=MessageProducer)
        mock_consumer = mocker.AsyncMock(spec=MessageConsumer)
        ...
        # Define workflow that handles PublishFailure ADT
        def publish_topic_not_found_program() -> Generator[AllEffects, EffectResult, str]:
            """Try to publish to nonexistent topic, handle via pattern matching."""
            publish_result = yield PublishMessage(topic="test-topic", payload=b"data")
        ...
        match result:
            case Ok(outcome):
                assert outcome == "failed: topic_not_found"
            case Err(error):
                pytest.fail(f"Expected Ok('failed: topic_not_found'), got Err({error})")
